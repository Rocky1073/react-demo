...
gulp.task("css", function() {
  ...
  return (
    gulp
      .src("src/scss/*.scss") // 指定css文件夹下的所有后缀为.css的文件
      .pipe(sass())
...
gulp.task("img", function() {
  gulp
    .src("dist/image/*.{png,jpg,gif,ico}")
    .pipe(
      imagemin({
    ...
    .pipe(gulp.dest("dist/images")); //输入到build文件夹下的images文件夹下
});
...
gulp.task("default", function() {
  ...
  //监听文件的变化实时编译 然后刷新
  gulp
    .watch([path.html, path.js, path.css, path.scss])
    .on("change", function() {
      gulp.start("css");
      browserSync.reload();
    });
